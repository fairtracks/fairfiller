...
import no.uio.ifi.model.ContentType
import no.uio.ifi.model.HubContent
...
val ONTOLOGIES: LoadingCache<String, String> =
    Caffeine.newBuilder().maximumSize(Long.MAX_VALUE).build { key -> getOntologyValue(key) }

fun main() {
    val resourceAsStream = object {}.javaClass.getResourceAsStream("/blueprint.json")
    val blueprint = GSON.fromJson(InputStreamReader(resourceAsStream), HubContent::class.java)
    ...
    for (track in blueprint?.tracks.orEmpty()) {
        ...
        if (track?.contentType == null) {
            track?.contentType = ContentType("http://edamontology.org/data_3002", "Annotation track")
        }
    }
